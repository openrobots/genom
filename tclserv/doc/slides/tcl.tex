...
\begin{part}{Introduction}{}
   ...
   \begin{bitemize}{color@bulle}
      \item a compiled language, efficient for the algorithms (cobol,
	    C, C++, JAVA),

      \item an interpreted language, used as glue to « pilot »
	    and personalized the application {\bf $\rightarrow$ TCL}.
   \end{bitemize}
   ...
   \begin{bitemize}{color@bulle}
      ...
      \item open-source free licence: TCL can be freely modified and
...
\begin{tslide}{Tcl and LAAS softwares}
   ...
   \begin{bitemize}{color@bulle}
      ...
      \item \textbf{Calife :} command language.
   \end{bitemize}
   \vfill
\end{tslide}
...
\begin{tslide}{Tcl Language}
   \vfill
   {\bf Numerous integrated functions :}
   \begin{bitemize}{color@bulle}
...
if "$x < 3" {
    puts "x is too small"
	    ...
	    \end{verbatim} %$
	 \end{quote}
      \item access to files, proces, sockets (network).
      \item procedures,
	 \begin{quote}
	    \begin{verbatim}
proc fac x {
...
\begin{tslide}{Data representation}
   ...
   From Tcl 8.3 : another type of data: binary data on 8 bits
   (example : images). 
   \vfill
\end{tslide}
...
result of the execution.
      ...
      \item \texttt{"blue bird"} \\
         ...
         Dollars and square brackets within are interpreted normally.
	 ...
	 caracters. 
      ...
      \item \verb|\| \\
	 introduced special characters  (\verb|\n|) and annhilates the
...
\begin{tslide}{Grouping and substitution}
   \vfill
   The TCL syntaxics analyser works in 3 steps:
   \begin{enumerate}
...
command as index in the array of the known procedures (preintegrated or
added by the uiser in C or TCL).
   \end{enumerate}
   \vfill
\end{tslide}
...
\begin{tslide}{Commented Example}
   ...
   \vfill
   \begin{bitemize}{color@bulle}
      \item 2 commands at 1$^{st}$ level : \texttt{set} and \texttt{while}
      \item 2 arguments for \texttt{while} : condition and body
      \item becarefull at the space between \texttt{\}} and \texttt{\{}
      \item automatical evaluation with \texttt{while} of the condition
...
\begin{tslide}{String operators}
   ...
   \vfill
   \begin{bitemize}{color@bulle}
      \item \verb|<|, \verb|>|, \verb|<=|, \verb|>=|, \verb|==| et \verb|!=|
	 work with strings:
      \item Becarefull with strings that look like numbers.
      \item One can used the function \texttt{string compare}.
   \end{bitemize}
   \vfill
\end{tslide}
...
\begin{tslide}{Lists}
   ...
   \begin{bitemize}{color@bulle}
	 ...
	 foreach linsert lrange lsort lappend list lreplace}
      ...
      \item The indices start at 0. \texttt{end} represents the last element.
   \end{bitemize}
   \vfill
   \textbf{Examples :}
   \begin{quote}
...
\begin{tslide}{"globbing" Globalisation}
   ...
   \end{bitemize}
   \vfill
   The command \texttt{glob} expanses a patern according to local files.
   \begin{quote}
   \begin{verbatim}
foreach f [glob *.exe] {
   puts "$f is a program"
   ...
   \vfill
   \begin{center}
      {\bf Do not mixed-up with regular expressions}
   \end{center}
   \vfill
\end{tslide}
...
\begin{tslide}{Regular expressions}
   \vfill
   Powerfull paterns description language. Similar to \texttt{sed}.\\
   ...
   \begin{tabular}{ll}
      ...
      \verb:|: & indiques an alternative between 2 expressions \\
   \end{tabular}
   \vfill
\end{tslide}
...
\begin{tslide}{Commands \texttt{format} and \texttt{scan}}
   ...
   \begin{bitemize}{color@bulle}
	 ...
	 \tclex{format "I know \%d Tcl commands" 97}{I know 97
	 Tcl commands}\\
...
\begin{tslide}{Control structures}
   ...
   \begin{bitemize}{color@bulle}
      ...
      \item \textbf{Example :} returns in \texttt{b} the list stocked
	 in \texttt{a}\\ 
	 \begin{verbatim}
	    set b ""
...
\begin{tslide}{control Structures : traps}
   \vfill
   Brackets are optionnal, but danger :
   \begin{quote}
      \begin{verbatim}
	 set x 3
	 if $x>2 { ...
...
scope
	 \item each procedure introduces a new name space
      \end{itemize}
   \item \texttt{global} allows to access to a global variable. 
...
strange effects:
	 \begin{verbatim}
	    proc affect {name valeur} {
	       global $name
	       set $name $valeur
	    }
	 \end{verbatim} %$

      \item \texttt{upvar} generalises this principle :
	 \begin{verbatim}
	    proc incr {name} {
	       upvar 1 $name var
	       set var [expr $var+1]
...
\begin{tslide}{Tcl naming spaces}
   \vfill
   \begin{bitemize}{color@bulle}
      \item For big softwares, it is necessary to  structure the
	    code $\rightarrow$ naming spaces (\emph{namespaces}). 
...
\begin{tslide}{Naming spaces : example}
   \vfill
   \begin{quote}
      \begin{verbatim}
...
\begin{tslide}{Access to files}
   ...
   \begin{bitemize}{color@bulle}
	 ...
	 source eof pwd filename}
      \item Commands use key words to design the open files.
     \begin{quote}
...
\begin{tslide}{Inputs/outputs and processes}
   \vfill
   \begin{bitemize}{color@bulle}
   ...
   \item \texttt{open} allows to create  tubes :
      \begin{quote}
	 \begin{verbatim}
...
\begin{tslide}{Execution  context informations}
   ...
   \begin{bitemize}{color@bulle}
	 ...
	 nombre, \texttt{argv} is the list of the arguments, \texttt{argv0}
	 ...
	 with th indices \texttt{osVersion}, \texttt{machine},
	 \texttt{platform}, \texttt{os}.

      \item the command \texttt{info} :
	 \begin{itemize}
	    \item on the variables : \texttt{info vars},
	    \texttt{info globals}, \texttt{info locals},
	    \texttt{info exists}

	    \item on the procedures : \texttt{info procs}, \texttt{info args},
...
\begin{tslide}{Code loading in the interpreter}
   \vfill
   \begin{bitemize}{color@bulle}
...
\begin{part}{Extend Tcl}{The philosophy}
   \vfill
   \begin{bitemize}{color@bulle}
      \item Why writting Tcl scripts rather C :
         \begin{itemize}
...
\begin{tslide}{New C commands}
   ...
   \begin{bitemize}{color@bulle}
      \item Initialisation function module \texttt{MyModule} :
      \begin{verbatim}
...
\begin{tslide}{Creation of Tcl modules}
   ...
   \begin{bitemize}{color@bulle}
      ...
      \begin{verbatim}
	 cc -c -Kpic -g MyModule.c
	 ld -G -z text MyModule.o -o MyModule.so
      \end{verbatim}
...
\begin{part}{}{Tcl at LAAS}
   \vfill
   \begin{bitemize}{color@bulle}
      \item Current version : 8.4.3 in \texttt{/usr/local/tcl-8.4}
...
%	    \item \texttt{file:/home/mallet/help/tcl8.3.1/html/contents.htm}
	 \end{itemize}
   \end{bitemize}
   \vfill
\end{part}
