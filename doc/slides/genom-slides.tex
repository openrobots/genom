...
\author{Sara Fleury - Anthony Mallet - Matthieu Herrb}
...
\texttt{pocolibs} (\url{https://www.openrobots.org/wiki/pocolibs})
...
\hline
\tt C-c C-m & \textbf{m}odule creation (\emph{1st command to call})\\
...
\begin{itemize}
\item \textbf{One main service}: "move the mobile to a given position"\\
$\Rightarrow$ \textbf{execution request}: \texttt{Goto}
%\vspace{-.8cm}
		\begin{itemize}
		\item input: \texttt{position} to reach
		\end{itemize}
\item \textbf{parameters control}: "get or change speed reference"\\
	 $\Rightarrow$ \textbf{control request}:
		\begin{itemize}
...
request SetSpeed {
    ...
    input_info:       DEMO_DEFAULT_SPEED::"DEMO_SLOW or "
                                          "DEMO_FAST";
...
3 files to describe a module:
	...
	\item structures definitions: \texttt{demoStruct.h}
	\item constants definitions: \texttt{demoConst.h}
	\end{itemize}
\vspace{1em}
...
\begin{enumerate}
\item Edit the module: \textbf{demo.gen}

\item Only the first time: generate \textbf{genom} \\
  and configure \textbf{configure}

\item Compile and generate: \textbf{make}

\item Install: \textbf{make install}
...
\textbf{(then one can restart the module)}
...
\item Only the first time: generate (\textbf{genom}) and configure (\textbf{configure})

\item (Re) generate and compile: \textbf{make}

\item Install: \textbf{make install}

\item Execute: \textbf{h2 init, demo -b, demoTest 1, ... h2 end}
...
\item All the codels are in the directory: \texttt{codels/}
...
\begin{itemize}
\item \texttt{demoCntrlTaskCodels.c} for the control codels (\texttt{c\_control\_func})

\item \texttt{demoMotionTaskCodels.c} for the execution codels
(\texttt{c\_exec\_func}, \ldots)
...
STATUS demoSetSpeedCntrl(DEMO_SPEED *speed, int *report)
{
   ...
   /* Parameter is valid:
      it will be recorded into the fIDS */
   return OK;
}
...
\bi
\item Initialisation codel \texttt{c\_init\_func}
\item Terminaison codel  \texttt{c\_end\_func}
\item Permanent activity codel: \texttt{c\_func}
\ei
...
\item Fill-in the codels (and goto \textbf{3})
...
\item On Interruption $\Rightarrow$ execution of the \texttt{inter} codel
\ei
...
\bi
\item usually 1 tic = 10ms on Linux
...
\begin{lstlisting}
ACTIVITY_EVENT
demoGotoStart(double *posRef, int *report)
{
   printf("Start engin\n");
...
Library: \texttt{demoPosterReadLib}

{\fontsize{6.95}{8}\selectfont
\begin{lstlisting}
...
\begin{itemize}
\item \texttt{tclServ} allows to control a set of modules from a unique interface
...
\item Graphical libraries \texttt{Tk}
\end{itemize}
...
\begin{enumerate}
\item Generate the module with option \texttt{-t}

\item Start the server \texttt{tclserv}.
...
command		& function \\
\hline
\tt replyof \$rqstId & get answer \emph{blocking mode}.
Generaly used with option \texttt{-raw}. \\
\tt cs::term \$rqstId 0 & get answer \emph{ non blocking mode}. \\
...
\begin{itemize}
\item Supervision
\item Generate the module with option \texttt{-o}
\end{itemize}
...
nom		& function \\
\hline
\tt DEMO-MOTION & send the request  \emph{and} receive the
                  \emph{final reply in blocking mode} \\
...
\\
\texttt{(FR DEMO DEMO\_MOTION \$RQST-ID \$REPORT \$DATA)}.

\end{frame}

%----------------------------------------------------------------------
\begin{frame}[containsverbatim]
\frametitle{Module control with OpenPRS: \texttt{transgen}}
...
\begin{itemize}
\item  Description  file \texttt{manip.tg}:
\end{itemize}
...
%----------------------------------------------------------------------
\begin{frame}[containsverbatim]
\frametitle{Module control with OpenPRS: \texttt{transgen} (continued)}

\begin{itemize}
\item Call: \texttt{tranGen manip.tg}
\item Compile in \texttt{server/}.
\item Start OpenPRS:
\end{itemize}

{\footnotesize
\begin{lstlisting}
blues% server/i386-linux/manip-xOpenPRS -n manip \
...
the continuous
localization request  (\texttt{SEGLOC-LOCLOOP-ASYNC}) when the
robot losts itself:
...
\item First time only: generate (\textbf{genom}) and configure (\textbf{configure})
...
\item Control with: \textbf{tclserv} and \textbf{Tcl/Tk} \texttt{(eltclsh, elwish,
gdhe)}	or \textbf{OpenPRS}
...
% LocalWords:  softs laas fr openrobots  codels usr PKG ONFIG ATH cd
